...
add_job()
{
  ...
  cp $NAMEPREFIX\_TEMPLATE.sh $NAMEPREFIX\_$MODELNAME.sh
  echo "Add task spooler jobs for $MODELNAME to the task spooler"
  echo "Shell script $NAMEPREFIX\_$MODELNAME.sh"
  ts -L AW_$MODELNAME $CURRENTFOLDER/$NAMEPREFIX\_$MODELNAME.sh
}
...
NAMEPREFIX=tf2oda_inf_eval_saved_model
